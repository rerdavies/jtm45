...
void wdfTree::cycleWave( ) {
    for( unsigned int i = 0; i < subtreeCount; i++ ) {
        rt_float wave = subtreeEntryNodes[i]->pullWaveUp( );
...
void wdfTree::initTree( ) {
    ascendingWaves.reset( new Col<rt_float>( subtreeCount ) );
    descendingWaves.reset( new Col<rt_float>( subtreeCount ) );
...
//----------------------------------------------------------------------
void wdfTree::setSamplerate( rt_float fs ) {
    treeSampleRate  = fs;
}

//----------------------------------------------------------------------
rt_float wdfTree::getSamplerate( ) {
...
//----------------------------------------------------------------------
void wdfRoot::setPortResistances( rt_float *Rp ) {
...
//----------------------------------------------------------------------
void wdfRootRtype::processAscendingWaves( Col<rt_float>* ascendingWaves,
                                          Col<rt_float>* descendingWaves ) {
...
//----------------------------------------------------------------------
void wdfRootNL::processAscendingWaves( Col<rt_float>* ascendingWaves,
                                       Col<rt_float>* descendingWaves ) {
...
//----------------------------------------------------------------------
void wdfRootSimple::setPortResistances( rt_float * Rp ) {
    rootElement->setPortResistance( Rp[0] );
}

//----------------------------------------------------------------------
void wdfRootSimple::processAscendingWaves( Col<rt_float>* ascendingWaves,
                                           Col<rt_float>* descendingWaves ) {
    size_t idx = 0;
...
//----------------------------------------------------------------------
rt_float wdfPort::getPortVoltage( ) {
    return ( a + b ) / 2.0;
}

//----------------------------------------------------------------------
rt_float wdfPort::getPortCurrent( ){
    return ( a - b ) / ( 2.0 * Rp );
}
...
//----------------------------------------------------------------------
rt_float wdfTreeNode::adaptPorts( rt_float sampleRate ) {
...
//----------------------------------------------------------------------
rt_float wdfTreeNode::pullWaveUp( ) {
    for( wdfPort* downPort : downPorts ) {
...
//----------------------------------------------------------------------
void wdfTreeNode::pushWaveDown( rt_float descendingWave ) {
...
wdfTerminatedRtype::wdfTerminatedRtype( std::vector<wdfTreeNode*> childrenIn ) : wdfTerminatedAdapter( childrenIn ) {
    S.reset( new Mat<rt_float>( childrenIn.size()+1, childrenIn.size()+1 ) );
}
...
// //----------------------------------------------------------------------
// rt_float wdfTerminatedRtype::calculateUpRes( rt_float sampleRate )
// {
//     const rt_float Rup = YOUR CODE HERE
//     return ( Rup );
// }
//
// //----------------------------------------------------------------------
...
//----------------------------------------------------------------------
rt_float wdfTerminatedRtype::calculateUpB( )
{
    ...
    rt_float upB = as_scalar( S->row(0) * inWaves );

    return upB;
}

//----------------------------------------------------------------------
void wdfTerminatedRtype::calculateDownB( rt_float descendingWave )
{
    colvec inWaves( childrenNodes.size()+1 );
...
//----------------------------------------------------------------------
rt_float wdfTerminatedSeries::calculateUpRes( rt_float sampleRate ) {
    const rt_float Rleft  = downPorts[0]->Rp;
    const rt_float Rright = downPorts[1]->Rp;
    const rt_float Rser   = Rleft + Rright;
    return ( Rser );
}

//----------------------------------------------------------------------
void wdfTerminatedSeries::calculateScatterCoeffs( ) {
    const rt_float Ru = upPort->Rp;
    const rt_float Rl = downPorts[0]->Rp;
    const rt_float Rr = downPorts[1]->Rp;
...
//----------------------------------------------------------------------
rt_float wdfTerminatedSeries::calculateUpB( ) {
    rt_float upB = -( downPorts[0]->a + downPorts[1]->a );
    return upB;
}

//----------------------------------------------------------------------
void wdfTerminatedSeries::calculateDownB( rt_float descendingWave ) {
...
//----------------------------------------------------------------------
rt_float wdfTerminatedParallel::calculateUpRes( rt_float sampleRate ) {
    const rt_float Rleft  = downPorts[0]->Rp;
    const rt_float Rright = downPorts[1]->Rp;
    ...
    const rt_float Rpar   = ( Rleft * Rright ) / ( Rleft + Rright );
    return Rpar;
}

//----------------------------------------------------------------------
void wdfTerminatedParallel::calculateScatterCoeffs( ) {
    const rt_float Gu = 1.0 / upPort->Rp;
    const rt_float Gl = 1.0 / downPorts[0]->Rp;
    const rt_float Gr = 1.0 / downPorts[1]->Rp;
...
//----------------------------------------------------------------------
rt_float wdfTerminatedParallel::calculateUpB( ) {
    return ( dl * downPorts[0]->a + dr * downPorts[1]->a );
}

//----------------------------------------------------------------------
void wdfTerminatedParallel::calculateDownB( rt_float descendingWave ) {
...
//----------------------------------------------------------------------
rt_float wdfInverter::calculateUpRes( rt_float sampleRate ) {
...
//----------------------------------------------------------------------
rt_float wdfInverter::calculateUpB( ) {
    return -1 * downPorts[0]->a ;
}

//----------------------------------------------------------------------
void wdfInverter::calculateDownB( rt_float descendingWave ) {
...
//==============================================================================
wdfTerminatedCap::wdfTerminatedCap( rt_float C,
                                    rt_float sampleRate ) : wdfTerminatedLeaf( ),
                                                 C( C ),
...
//----------------------------------------------------------------------
rt_float wdfTerminatedCap::calculateUpRes( rt_float sampleRate ) {
    ...
    const rt_float R = 1 / ( 2.0 * sampleRate * C );
    return R;
}

//----------------------------------------------------------------------
rt_float wdfTerminatedCap::calculateUpB( ) {
    return prevA;
}

//----------------------------------------------------------------------
void wdfTerminatedCap::calculateDownB( rt_float descendingWave ) {
...
//==============================================================================
wdfTerminatedInd::wdfTerminatedInd( rt_float L,
                                    rt_float sampleRate ) : wdfTerminatedLeaf( ),
                                                 L( L ),
                                                 sampleRate( sampleRate ),
                                                 prevA( 0 ) {
...
//----------------------------------------------------------------------
rt_float wdfTerminatedInd::calculateUpRes( rt_float sampleRate ) {
    assert(sampleRate > 0 && "sampleRate must be a nonzero positive number.");
    assert(L > 0 && "inductance must be a nonzero positive number.");

    this->sampleRate = sampleRate;
    const rt_float R = 2.0 * sampleRate * L ;
    return R;
}

//----------------------------------------------------------------------
rt_float wdfTerminatedInd::calculateUpB( ) {
    return prevA;
}

//----------------------------------------------------------------------
void wdfTerminatedInd::calculateDownB( rt_float descendingWave ) {
...
//==============================================================================
wdfTerminatedRes::wdfTerminatedRes( rt_float R ) : wdfTerminatedLeaf( ),
                                                 R( R ) {

}

//----------------------------------------------------------------------
rt_float wdfTerminatedRes::calculateUpRes( rt_float sampleRate ) {
    return R;
}

//----------------------------------------------------------------------
rt_float wdfTerminatedRes::calculateUpB( ) {
    return 0.0;
}

//----------------------------------------------------------------------
void wdfTerminatedRes::calculateDownB( rt_float descendingWave ) {
...
//==============================================================================
wdfTerminatedResVSource::wdfTerminatedResVSource( rt_float Vs,
                                                  rt_float RSer ) : wdfTerminatedLeaf( ),
                                                                  Vs( Vs ),
                                                                  RSer( RSer ) {
...
//----------------------------------------------------------------------
rt_float wdfTerminatedResVSource::calculateUpRes( rt_float sampleRate ) {
    return RSer;
}

//----------------------------------------------------------------------
rt_float wdfTerminatedResVSource::calculateUpB( ) {
    return Vs;
}

//----------------------------------------------------------------------
void wdfTerminatedResVSource::calculateDownB( rt_float descendingWave ) {
...
//==============================================================================
wdfTerminatedResCSource::wdfTerminatedResCSource( rt_float Is,
                                                  rt_float RPar ) : wdfTerminatedLeaf( ),
                                                                  Is( Is ),
                                                                  RPar ( RPar ) {
...
//----------------------------------------------------------------------
rt_float wdfTerminatedResCSource::calculateUpRes( rt_float sampleRate ) {
    return RPar;
}

//----------------------------------------------------------------------
rt_float wdfTerminatedResCSource::calculateUpB( ) {
    return RPar * Is;
}

//----------------------------------------------------------------------
void wdfTerminatedResCSource::calculateDownB( rt_float descendingWave ) {
...
//----------------------------------------------------------------------
void wdfRootNode::setPortResistance( rt_float Rp ) {
    this->Rp = Rp;
}
...
//----------------------------------------------------------------------
void wdfUnterminatedSwitch::calculateDownB( Col<rt_float>* ascendingWaves,
                                            Col<rt_float>* descendingWaves,
                                            size_t* portIndex ) {
...
//==============================================================================
wdfUnterminatedCap::wdfUnterminatedCap(rt_float C,
                                       rt_float sampleRate ) : wdfRootNode(1),
                                                    sampleRate(sampleRate),
                                                    prevA(0),
                                                    prevB(0),
                                                    C(C) {
...
//----------------------------------------------------------------------
void wdfUnterminatedCap::calculateDownB( Col<rt_float>* ascendingWaves,
                                         Col<rt_float>* descendingWaves,
                                         size_t* portIndex) {
...
void wdfUnterminatedCap::setPortResistance( rt_float Rp ) {
    this->Rp = Rp;
...
//==============================================================================
wdfUnterminatedInd::wdfUnterminatedInd( rt_float L,
                                        rt_float sampleRate ) : wdfRootNode(1),
                                                     sampleRate(sampleRate),
                                                     prevA(0),
                                                     prevB(0),
...
//----------------------------------------------------------------------
void wdfUnterminatedInd::calculateDownB( Col<rt_float>* ascendingWaves,
                                         Col<rt_float>* descendingWaves,
                                         size_t* portIndex) {
    descendingWaves->at(*portIndex) = -reflectionCoeff * prevB - reflectionCoeff * ascendingWaves->at(*portIndex) - prevA;
    prevB = descendingWaves->at(*portIndex);
...
void wdfUnterminatedInd::setPortResistance( rt_float Rp ) {
    this->Rp = Rp;
...
//==============================================================================
wdfUnterminatedRes::wdfUnterminatedRes( rt_float R ) : wdfRootNode(1),
                                                     R(R) {

}

//----------------------------------------------------------------------
void wdfUnterminatedRes::calculateDownB( Col<rt_float>* ascendingWaves,
                                         Col<rt_float>* descendingWaves,
                                         size_t* portIndex) {
...
void wdfUnterminatedRes::setPortResistance( rt_float Rp ) {
    this->Rp = Rp;
...
//==============================================================================
wdfIdealVSource::wdfIdealVSource( rt_float Vs ) : wdfRootNode(1),
                                                Vs(Vs) {

}

//----------------------------------------------------------------------
void wdfIdealVSource::calculateDownB( Col<rt_float>* ascendingWaves,
                                            Col<rt_float>* descendingWaves,
                                            size_t* portIndex) {
...
//----------------------------------------------------------------------
void wdfIdealVSource::setPortResistance( rt_float Rp ) {
    this->Rp = Rp;
}

#pragma mark Ideal Current Source
//==============================================================================
wdfIdealCSource::wdfIdealCSource( rt_float Is ) : wdfRootNode(1),
                                                  Is(Is) {

}

//----------------------------------------------------------------------
void wdfIdealCSource::calculateDownB( Col<rt_float>* ascendingWaves,
                                      Col<rt_float>* descendingWaves,
                                      size_t* portIndex) {
...
//----------------------------------------------------------------------
void wdfIdealCSource::setPortResistance( rt_float Rp ) {
    this->Rp = Rp;
}
